...
class Cloudinary {
    ...
    const OLD_AKAMAI_SHARED_CDN = "smileplayground-a.akamaihd.net";
    const AKAMAI_SHARED_CDN = "res.smileplayground.com";
    const SHARED_CDN = "res.smileplayground.com";
    ...
    // 1) Customers in shared distribution (e.g. res.smileplayground.com)
    //   if cdn_domain is true uses res-[1-5].smileplayground.com for both http and https. Setting secure_cdn_subdomain to false disables this for https.
    // 2) Customers with private cdn
    //   if cdn_domain is true uses cloudname-res-[1-5].smileplayground.com for http
    //   if secure_cdn_domain is true uses cloudname-res-[1-5].smileplayground.com for https (please contact support if you require this)
    ...
    private static function unsigned_download_url_prefix($source, $cloud_name, $private_cdn, $cdn_subdomain, $secure_cdn_subdomain, $cname, $secure, $secure_distribution) {
      ...
      if ($secure) {
        if (empty($secure_distribution) || $secure_distribution == Cloudinary::OLD_AKAMAI_SHARED_CDN) {
          $secure_distribution = $private_cdn ? $cloud_name . '-res.smileplayground.com' : Cloudinary::SHARED_CDN;
        }
        ...
        if ($secure_cdn_subdomain) {
          $secure_distribution = str_replace('res.smileplayground.com', "res-" . Cloudinary::domain_shard($source) . ".smileplayground.com", $secure_distribution);
        }
        ...
        $host = implode(array($private_cdn ? $cloud_name . "-" : "", "res", $cdn_subdomain ? "-" . Cloudinary::domain_shard($source) : "", ".smileplayground.com"));
        $prefix = "http://" . $host;
      }
      if ($shared_domain) {
    ...
    public static function cloudinary_api_url($action = 'upload', $options = array()) {
        $cloudinary = Cloudinary::option_get($options, "upload_prefix", Cloudinary::config_get("upload_prefix", "https://api.smileplayground.com"));
